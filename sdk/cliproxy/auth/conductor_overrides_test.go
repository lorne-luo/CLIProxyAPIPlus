...
func TestManager_ModelUnavailableBadRequest_CoolsDownAuthForTenMinutes(t *testing.T) {
	m := NewManager(nil, nil, nil)
	executor := &authFallbackExecutor{
		id: "claude",
		executeErrors: map[string]error{
			"aa-bad-auth": &Error{
				HTTPStatus: http.StatusBadRequest,
				Message:    "invalid_request_error: The requested model is unavailable.",
			},
		},
	}
	m.RegisterExecutor(executor)
	...
	if state.NextRetryAfter.IsZero() {
		t.Fatalf("expected bad auth model %q to be suspended for temporary unavailable error", model)
	}
	remaining := time.Until(state.NextRetryAfter)
	if remaining < 9*time.Minute || remaining > 10*time.Minute+5*time.Second {
		t.Fatalf("expected temporary unavailable cooldown near 10 minutes, got remaining=%v", remaining)
	}
}

func TestManagerExecuteStream_ModelUnavailableBadRequest_CoolsDownAuthForTenMinutes(t *testing.T) {
	m := NewManager(nil, nil, nil)
	executor := &authFallbackExecutor{
		id: "claude",
		streamFirstErrors: map[string]error{
