	...
	if auth, ok := m.auths[result.AuthID]; ok && auth != nil {
		...
		if result.Success {
			...
			if result.Model != "" {
				...
				} else if result.Error != nil && isModelTemporarilyUnavailableMessage(result.Error.Message) {
					next := now.Add(10 * time.Minute)
					state.NextRetryAfter = next
					suspendReason = "model_temporarily_unavailable"
					shouldSuspendModel = true
				} else {
					switch statusCode {
					case 401:
...
func isModelTemporarilyUnavailableMessage(message string) bool {
	lower := strings.ToLower(strings.TrimSpace(message))
	if lower == "" {
		return false
	}
...
func isModelSupportError(err error) bool {
	...
	if isModelTemporarilyUnavailableMessage(err.Error()) {
		return false
	}
...
func isModelSupportResultError(err *Error) bool {
	...
	if isModelTemporarilyUnavailableMessage(err.Message) {
		return false
	}
